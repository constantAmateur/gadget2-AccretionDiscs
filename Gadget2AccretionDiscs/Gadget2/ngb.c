...
int ngb_treefind_variable_2D(FLOAT searchcenter[3], FLOAT hsml, int *startnode)
{
  ...
  while(no >= 0)
      ...
      if(no < All.MaxPart)	/* single particle */
	{
	  ...
	  if(numngb == MAX_NGB)
	    {
	      numngb = ngb_clear_buf_2D(searchcenter, hsml, numngb);
	      if(numngb == MAX_NGB)
		{
		  printf("ThisTask=%d: Need to do a second neighbour loop for (%g|%g) hsml=%g no=%d\n",
			 ThisTask, searchcenter[0], searchcenter[1], hsml, no);
		  *startnode = no;
		  return numngb;
		}
	    }
 ...
 */
int ngb_clear_buf_2D(FLOAT searchcenter[2], FLOAT hsml, int numngb)
{
  int i, p;
  FLOAT dx, dy, r2;
