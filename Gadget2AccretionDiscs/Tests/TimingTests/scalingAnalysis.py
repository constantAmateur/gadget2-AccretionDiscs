...
for i in xrange(len(dat)):
  ...
  daty=[]
  for x in dat[i][3]:
    mm=min(x[0].shape[0],dat[i][2][0].shape[0])
    t=x[0][:mm,1]
    first=(x[0][:mm,what])
    gs=(dat[i][2][0][:mm,what])
...
for tmp in plotDat:
  ...
  if len(tmp[0])>len(speedup.bigx):
    speedup.bigx=tmp[0]
  efficiency.plot(np.log2(tmp[0]),np.array(tmp[1])/np.array(tmp[0]),label="No. pcles="+str(tmp[2]))
...
for what in xrange(2,12):
  ...
  for x in timeData:
    ...
    if what>8:
      style='--'
  plt.plot(np.log2(datx),np.array(daty)/np.array(datx),style)
