...
#This is normalized so that Q is 2 at the outer edge of the disc...
Qmin_init=2.0
#Calculate what the base temperature will be for this choice...
sigma0=(M_disk*(plIndex+2))/(2*pi*(r_outer^(plIndex+2)-r_inner^(plIndex+2)))
omega_outer=sqrt((ginternal*M_star)/r_outer^3)
sigma_outer=sigma0*(r_outer^plIndex)
cs_outer=sqrt((gamma*k_b*(r_outer^temppower))/(mu*m_H*(r_inner^temppower)))
baseTemp= ((Qmin_init*sigma_outer*pi*ginternal)/(cs_outer*omega_outer))^2
