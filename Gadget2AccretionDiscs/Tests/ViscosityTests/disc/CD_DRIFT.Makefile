...
#
# - ALT_DIVV
#     This uses the method of Cullen and Dehnen ,10 to estimate div.v,
#     instead of the standard method used by GADGET.  This is NOT a full
#     implementation of the AV method in their paper as this would require 
#     considerable additional computational overhead.  The only difference 
#     between the implemented method and the one in the paper is that the 
#     weights \bar{w_{ij}} depends on p_i rather than p_j.  The same thing
#     is done by GADGET in the standard estimator of div.v and is done to 
#     avoid having to do an additional loop over neighbours.  The primary 
#     motivation for improving the estimator of div.v is to try and mitigate
#     the issues raised in Cartwright and Stamatellos, 2010 regarding the 
#     affect of artificial viscosity on accretion discs, despite the use of
#     switches designed to prevent any unwanted viscosity effects.  Turning 
#     this off may save some computational time if you know this will not
#     be an issue.
#
# - PRICE_GRAV_SOFT
#     When adaptive gravitational smoothing lengths are used for gas particles, 
#     the energy conservation properties of GADGET are broken by default.  To 
#     correct this, it is necessary to add extra terms to the acceleration
#     as described by Price and Monaghan, 2006.  When this option is on these
#     additional terms are included.
#
# - VARIABLE_VISC_CONST
#     This switch allows each particle to have its own value of the artificial
#     viscosity parameter alpha.  These are then evolved according to the method
#     of Morris and Monaghan.  When this option is enabled, the parameter file 
#     option ArtBulkViscConst now specifies a MAXIMUM viscosity and two additional
#     parameters are needed: VariableViscAlphaMin, which gives the minimum viscosity
#     and VariableViscDecayLength, which controls how quickly alpha decays to alpha_min
#     after a shock.
#
# - OUTPUTAPLHA
#     When the above option is on, outputs the individual particle viscosity alpha values.
#
# - SINK_PARTICLES
#     Any particle of type 1 will be marked as a sink particle.  Any gas particle which
#     is closer than AccretionRadius (parameter file option) and with less kinetic energy
#     than the sink has gravitational potential at that point will be accreted.  
#     During accretion the mass and momentum of the gas particles is added to the sink.
#
# - NO_ACC_TEST
#     Only works with the above option.  Any particle within the accretion radius will 
#     be accreted regardless of its energy or angular momentum.
#
# - NOFFTWINSTALLED
#     Allows the code to compile even if FFTW is not installed.  Will not work if an 
#     option is selected which needs FFTW such as PMGRID.
# - VERBOSE
#     Prints out various diagnostic messages.
#-----------------------------------------------------------------------
